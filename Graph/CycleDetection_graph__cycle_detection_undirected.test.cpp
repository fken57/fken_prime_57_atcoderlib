...
// problem: graph/cycle_detection_undirected
...
// Injecting Graph/CycleDetection.hpp <- _fib/Graph/CycleDetection_graph__cycle_detection_undirected.test.cpp
...
int main() {
    ...
    const GraphCycle cycle = find_undirected_cycle(n, edges);
    if (!cycle) {
        cout << -1 << '\n';
        return 0;
    }
    cout << cycle.vertices.size() << '\n';
    for (int i = 0; i < static_cast<int>(cycle.vertices.size()); ++i) {
        if (i) cout << ' ';
        cout << cycle.vertices[i];
    }
    cout << '\n';
    for (int i = 0; i < static_cast<int>(cycle.edges.size()); ++i) {
        if (i) cout << ' ';
        cout << cycle.edges[i];
    }
    cout << '\n';
