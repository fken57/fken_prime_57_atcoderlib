...
// problem: number_theory/factorize
...
// Injecting math/PrimeFactorization64.hpp <- _fib/math/PrimeFactorization64_number_theory__factorize.test.cpp
...
int main() {
    ...
    while (query_count--) {
        ...
        const auto factors = factorize_u64(value);
        cout << factors.size();
        for (const uint64_t factor : factors) cout << ' ' << factor;
        cout << '\n';
    }
}
